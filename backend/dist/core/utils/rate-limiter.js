...
exports.RATE_LIMIT_TIERS = void 0;
...
function getTierForPath(pathname) {
    if (pathname.startsWith('/api/quiz/verify'))
        return exports.RATE_LIMIT_TIERS.QUIZ_VERIFY;
    if (pathname.startsWith('/api/quiz/start'))
        return exports.RATE_LIMIT_TIERS.QUIZ_START;
    if (pathname.startsWith('/api/send-email'))
        return exports.RATE_LIMIT_TIERS.SEND_EMAIL;
    if (pathname.startsWith('/api/admin/bulk-create-users'))
        return exports.RATE_LIMIT_TIERS.BULK_CREATE;
    if (pathname.startsWith('/api/sync-ieee-events'))
        return exports.RATE_LIMIT_TIERS.SYNC_EVENTS;
    if (pathname.startsWith('/auth/callback'))
        return exports.RATE_LIMIT_TIERS.AUTH_CALLBACK;
    if (pathname === '/login')
        return exports.RATE_LIMIT_TIERS.LOGIN_PAGE;
    return exports.RATE_LIMIT_TIERS.GENERAL;
}
// ---------------------------------------------------------------------------
// Fingerprint Builder
// ---------------------------------------------------------------------------
 ...
 */
async function buildFingerprint(ip, userAgent, acceptLanguage, userId) {
    const raw = userId
        ? `uid:${userId}`
 ...
 */
async function checkRateLimit(fingerprint, tier) {
    ...
    try {
        const result = await redis.eval(SLIDING_WINDOW_SCRIPT, [key], [now.toString(), windowMs.toString(), tier.maxRequests.toString(), (tier.windowSeconds + 10).toString()]);
